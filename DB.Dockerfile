...
RUN echo "#!/bin/bash\n\
psql -U postgres -c \"CREATE DATABASE blockchain_node WITH ENCODING = 'UTF8';\"\n\
psql -U postgres -c \"SET synchronous_commit TO off;\"\n\
psql -U postgres -d blockchain_node -v node_username=$NODENAME -v node_password=$NODEPASS -v network_username=$NETWORKNAME \
	-v network_password=$NETWORKPASS -v backend_username=$BACKENDNAME -v backend_password=$BACKENDPASS -f /docker-entrypoint-initdb.d/FullSetupDB" > /docker-entrypoint-initdb.d/FullSetupDB.sh